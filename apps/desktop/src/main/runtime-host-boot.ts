...
function registerHostClientIpc(
  ...
  // DesktopRuntimeHostClient exposes the full direct request surface, while
  // the plugin bridge narrows it to its five plugin operations. The runtime
  // client accepts these operations at runtime; keep the narrower adapter
  // local to this bridge instead of weakening the client API globally.
  const pluginClient = client as unknown as ClientPluginQueryClient & ClientPluginRemoteClient;
  registerClientPluginIpc({ ipcMain: scopedIpc, client: pluginClient, transport: clientPluginTransport });
  ...
  return async () => {
    clientPluginTransport.release(pluginClient);
